...
signal plane_down(plane_id, killer)
signal upgrade(field_name)
signal update_kills(value, color)
...
func _on_Biplane_dead(plane_id:int, killer_id:int)->void:
	emit_signal("plane_down", plane_id, killer_id)


func score(dead_color:Color)->void:
	kills += 1
	emit_signal("update_kills", kills, dead_color)
...
func _on_Biplane_update_airspeed(value:float)->void:
	emit_signal("update_airspeed", value)
